...
import { VmsIconBadge } from "./VmsPage";
...
export function BadgePreview({
  ...
  return (
    <div className={cn("rounded-lg border border-border bg-card p-4 text-card-foreground shadow-sm", className)}>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          {visitor.photo ? (
            ...
            <VmsIconBadge tone="strong" className="h-12 w-12 text-sm font-semibold">
              {getInitials(visitor.name)}
            </VmsIconBadge>
          )}
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-foreground">{visitor.name}</p>
            <p className="truncate font-mono text-xs text-muted-foreground">{visitor.visitorId || visitor.id}</p>
          </div>
        </div>
      ...
      <div className={cn("grid gap-4", compact ? "grid-cols-1" : "sm:grid-cols-[132px_1fr]")}>
        <div className="flex justify-center rounded-lg border border-border bg-muted/40 p-4">
          <QRCode value={payload} size={compact ? 104 : 120} bgColor="#ffffff" fgColor="#020617" />
        </div>
