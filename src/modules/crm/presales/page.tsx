...
function LeadKanbanCard({
  ...
  return (
    <button
      ...
      className="group flex w-full flex-col gap-2 rounded-2xl border border-slate-200/80 bg-white px-4 py-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-md"
    >
...
function KanbanColumn({
  ...
  return (
    <div className="flex min-w-[260px] min-h-[360px] flex-1 flex-col gap-3 rounded-[24px] border border-slate-200/80 bg-white/95 p-4 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            <span className={`flex h-8 w-8 items-center justify-center rounded-2xl ${tone}`}>
              {icon}
            </span>
            <div>
        ...
        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600 shadow-sm">
          {leads.length}
        </span>
      </div>
...
export default function PreSalesOverviewPage() {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-6 bg-slate-50/70 p-2 sm:p-4">
      <div className="rounded-[28px] border border-slate-200/80 bg-white/90 p-6 shadow-sm backdrop-blur">
        <div className="flex flex-wrap items-start justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 shadow-sm hover:border-slate-300 hover:bg-slate-50"
            >
            ...
            {stageSeries.map((item) => {
              ...
              return (
                <div key={item.status} className="flex items-center gap-3">
                  ...
                  <div className="h-2 flex-1 rounded-full bg-slate-100/80">
                    <div
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
        ...
        <span className="rounded-full bg-white/90 px-3 py-1 text-xs text-slate-600 shadow-sm">
          New: {data.statusCounts?.[LeadStatus.NEW] ?? 0}
        </span>
        <span className="rounded-full bg-white/90 px-3 py-1 text-xs text-slate-600 shadow-sm">
          Contacted: {data.statusCounts?.[LeadStatus.CONTACTED] ?? 0}
        </span>
        <span className="rounded-full bg-white/90 px-3 py-1 text-xs text-slate-600 shadow-sm">
          Qualified: {data.statusCounts?.[LeadStatus.QUALIFIED] ?? 0}
        </span>
        <span className="rounded-full bg-white/90 px-3 py-1 text-xs text-slate-600 shadow-sm">
